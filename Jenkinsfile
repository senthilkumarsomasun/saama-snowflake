...
export CLASSPATH=/etc/liqubase/lib/snowflake-jdbc-3.9.2.jar'''
        ...
        stage('OMLIQUBASEOM') {
          steps {
            sh '/etc/liqubase/liquibase update --url=\'jdbc:snowflake://${SF_URL_USR}?warehouse=DEMO_WH&db=DEV_CUST_DB&schema=CUSTOMER&&role=PUBLIC\' --changeLogFile=my_app-wrapper.xml --username=$SF_CRED_USR --password=$SF_CRED_PSW'
          }
        }
