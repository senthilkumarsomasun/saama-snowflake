...
export CLASSPATH=/etc/liqubase/lib/snowflake-jdbc-3.9.2.jar'''
            ...
            sh '''cd /etc/liqubase/;
liquibase update --url="jdbc:snowflake:saama.snowflakecomputing.com/?db=DEV_CUST_DB&schema=CUSTOMER" --changeLogFile=my_app-wrapper.xml --username=$SF_CRED_USR --password=$SF_CRED_PSW'''
          }
        }
